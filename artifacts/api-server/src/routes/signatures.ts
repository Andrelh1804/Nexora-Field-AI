...
router.get("/service-orders/:id", requireAuth, async (req: AuthRequest, res: Response) => {
  const orderId = Number(req.params["id"]);
  const sigs = await db.select().from(digitalSignaturesTable).where(eq(digitalSignaturesTable.serviceOrderId, orderId));
  res.json(sigs);
});

router.post("/service-orders/:id/sign", requireAuth, async (req: AuthRequest, res: Response) => {
  const orderId = Number(req.params["id"]);
  ...
  const [existing] = await db.select().from(digitalSignaturesTable)
    .where(and(
      ...
      eq(digitalSignaturesTable.signerId, req.userId!)
    ));

  const hash = crypto.createHash("sha256")
    .update(`${orderId}-${req.userId!}-${Date.now()}`)
    .digest("hex");
